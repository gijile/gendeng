...
public:
    ...
    std::vector<StorageDatabase> getStorageDatabases() const {
        return databases;
    }

    // Get items from specific database
    std::vector<StorageItem> getStorageItems(const std::string& dbId) const {
    ...
    StorageStats getStats() const {
        StorageStats stats = {0, 0, 0, 0, 0, 0, 0, 0, 0};
